...
export async function readStorage(
    ...
    const values = vars ? layout.storage.filter((v) => vars.includes(v.label)) : layout.storage;

    // create a parser and read values
    const parser = new StorageParser(getStorage, layout, mapKeys, parseBlockTag(block));
